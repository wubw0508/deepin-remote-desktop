 ...
 */
static void drd_x11_input_dispose(GObject *object) {
 ...
 * 外部接口：GLib 类型系统。
 */
static void drd_x11_input_class_init(DrdX11InputClass *klass) {
 ...
 */
static void drd_x11_input_init(DrdX11Input *self) {
 ...
 * 逻辑：若已打开则直接返回；调用 XOpenDisplay 获取连接并校验 XTest
 * 扩展；记录屏幕尺寸与编码流尺寸；通过 FreeRDP keyboard API
 * 初始化布局；最后计算指针缩放。 参数：self 输入实例；error 错误输出。
 * 外部接口：X11
 * XOpenDisplay/DefaultScreen/DisplayWidth/DisplayHeight、XTestQueryExtension
 * 校验扩展；FreeRDP freerdp_keyboard_init；GLib g_set_error_literal。
 */
static gboolean drd_x11_input_open_display(DrdX11Input *self, GError **error) {
 ...
 * 外部接口：drd_x11_input_open_display（X11/FreeRDP）；GLib
 * g_mutex_lock/unlock。
 */
 ...
 * 参数：self 输入实例。
 * 外部接口：XCloseDisplay（通过 drd_x11_input_close_display）；GLib
 * g_mutex_lock/unlock。
 */
 ...
 */
void drd_x11_input_update_desktop_size(DrdX11Input *self, guint width,
                                       guint height) {
  g_return_if_fail(DRD_IS_X11_INPUT(self));
 ...
 */
static gboolean drd_x11_input_check_running(DrdX11Input *self, GError **error) {
 ...
 * 逻辑：持锁校验运行态；解析扩展/释放标志并解析 RDP 扫描码到 X11 keycode（缓存
 * + FreeRDP 映射 + 特殊键查找）；注入 XTest FakeKeyEvent 并刷新。 参数：self
 * 输入实例；flags RDP 键盘标志；scancode RDP 基础扫描码；error 错误输出。
 * 外部接口：FreeRDP
 * MAKE_RDP_SCANCODE/RDP_SCANCODE_CODE/freerdp_keyboard_get_x11_keycode_from_scancode；XTestFakeKeyEvent/XFlush；日志
 * DRD_LOG_DEBUG。
 */
gboolean drd_x11_input_inject_keyboard(DrdX11Input *self, guint16 flags,
                                       guint8 scancode, GError **error) {
  g_return_val_if_fail(DRD_IS_X11_INPUT(self), FALSE);

  g_mutex_lock(&self->lock);
  if (!drd_x11_input_check_running(self, error)) {
    DRD_LOG_WARNING("Keyboard injection failed - input injector not running, "
                    "flags: 0x%04X, scancode: 0x%02X",
                    flags, scancode);
    g_mutex_unlock(&self->lock);
    return FALSE;
  }
  ...
  const gboolean extended =
      (flags & (KBD_FLAGS_EXTENDED | KBD_FLAGS_EXTENDED1)) != 0;
  ...
  XTestFakeKeyEvent(self->display, (unsigned int)x11_keycode,
                    release ? False : True, CurrentTime);
  XFlush(self->display);

  DRD_LOG_DEBUG("Keyboard injection - flags: 0x%04X, scancode: 0x%02X, "
                "release: %s, extended: %s, "
                "rdp_scancode: 0x%08X, base_scancode: 0x%02X, x11_keycode: %u, "
                "cache_miss: %s, action: %s",
                flags, scancode, release ? "true" : "false",
                extended ? "true" : "false", rdp_scancode, base_scancode,
                x11_keycode, cache_miss ? "true" : "false",
...
/*
 * 功能：注入 Unicode 键盘事件。
 * 逻辑：持锁检查运行态；将 Unicode 转为 KeySym 再转 keycode，利用 XTest
 * 注入按下/释放；无法映射时记录 debug 并返回成功以避免断流。 参数：self
 * 输入实例；flags RDP 键盘标志；codepoint Unicode 码点；error 错误输出。
 * 外部接口：XKeysymToKeycode、XTestFakeKeyEvent/XFlush；日志
 * DRD_LOG_DEBUG；内部 drd_x11_input_keysym_from_codepoint。
 */
gboolean drd_x11_input_inject_unicode(DrdX11Input *self, guint16 flags,
                                      guint16 codepoint, GError **error) {
  g_return_val_if_fail(DRD_IS_X11_INPUT(self), FALSE);
  ...
  XTestFakeKeyEvent(self->display, keycode, release ? False : True,
                    CurrentTime);
  XFlush(self->display);
 ...
 * 外部接口：无（内部计算）。
 */
static int drd_x11_input_pointer_button(guint16 flags, guint16 mask,
                                        int button_id) {
  if ((flags & mask) == 0) {
    return 0;
  }
...
/*
 * 功能：注入指针移动/按键/滚轮事件。
 * 逻辑：持锁检查运行态；按流/桌面尺寸计算缩放与裁剪后的坐标；根据标志注入移动、按键与滚轮事件，最后刷新
 * X11 输出。 参数：self 输入实例；flags RDP 指针标志；x/y 流坐标；error
 * 错误输出。 外部接口：XTestFakeMotionEvent/XTestFakeButtonEvent/XFlush；依赖
 * GLib MAX 宏；日志 DRD_LOG_DEBUG。
 */
gboolean drd_x11_input_inject_pointer(DrdX11Input *self, guint16 flags,
                                      guint16 x, guint16 y, GError **error) {
  g_return_val_if_fail(DRD_IS_X11_INPUT(self), FALSE);
  ...
  const guint16 max_stream_x =
      (guint16)(stream_width > 0 ? stream_width - 1 : 0);
  const guint16 max_stream_y =
      (guint16)(stream_height > 0 ? stream_height - 1 : 0);
  ...
  if (stream_width != desktop_width) {
    guint scaled =
        (guint)((gdouble)clamped_stream_x * self->stream_to_desktop_scale_x +
                0.5);
  ...
  if (stream_height != desktop_height) {
    guint scaled =
        (guint)((gdouble)clamped_stream_y * self->stream_to_desktop_scale_y +
                0.5);
  ...
  if (flags & PTR_FLAGS_MOVE) {
    XTestFakeMotionEvent(self->display, self->screen, target_x, target_y,
                         CurrentTime);
  }
  ...
  for (guint i = 0; i < G_N_ELEMENTS(button_map); ++i) {
    int button_event = drd_x11_input_pointer_button(flags, button_map[i].mask,
                                                    button_map[i].button_id);
    if (button_event > 0) {
 ...
 *   - extended = FALSE:
 * 映射到左键（XK_Alt_L/XK_Control_L/XK_Shift_L/XK_Super_L）
 *   - extended = TRUE:
 * 映射到右键（XK_Alt_R/XK_Control_R/XK_Shift_R/XK_Super_R）
 *
 * 参数：
 ...
 *
 * 详细说明：
 * 该函数是键盘输入转换的核心组件，负责将远程桌面协议（RDP）的键盘扫描码映射到本地
 * X11 系统的 keycode。 由于 RDP 和 X11
 * 使用不同的键盘编码体系，需要进行转换。为了提高性能，函数实现了缓存机制，
 * 避免每次按键事件都进行昂贵的映射计算。
 *
 * 缓存策略：
 ...
 * 2. 检查缓存是否命中
 * 3. 缓存未命中时：
 *    a. 对于修饰键（Ctrl/Alt/Shift/Win），优先使用 X11 的 XKeysymToKeycode
 * 进行映射 这是因为 FreeRDP 的键盘映射表可能与当前系统的 X11 键盘布局不一致 b.
 * 对于其他按键，使用 FreeRDP 的 freerdp_keyboard_get_x11_keycode_from_scancode
 * 进行标准映射 c. 如果标准映射失败（返回 0），尝试特殊键映射 d.
 * 将映射结果存入缓存
 ...
 *
 * 使用场景：
 *   在 drd_x11_input_inject_keyboard 函数中被调用，用于将 RDP 键盘事件转换为
 * X11 事件。 每次按键事件都会调用此函数，因此缓存机制对性能至关重要。
 */
static guint16 drd_x11_input_resolve_keycode(DrdX11Input *self,
                                             guint8 base_scancode,
                                             gboolean extended,
  ...
  if (cached == DRD_X11_KEYCODE_CACHE_INVALID) {
    ...
    // 步骤 1：对于修饰键（Ctrl/Alt/Shift/Win），优先使用 X11 的
    // XKeysymToKeycode 进行映射 这是因为 FreeRDP 的键盘映射表可能与当前系统的
    // X11 键盘布局不一致 例如：FreeRDP 可能返回左 Alt 的 keycode 为
    // 205，但实际系统的 keycode 是 64 注意：在 RDP
    // 协议中，左右修饰键使用相同的扫描码，通过 extended 标志区分
    switch (base_scancode) {
    case RDP_SCANCODE_CODE(
        RDP_SCANCODE_LMENU): // 左/右 Alt（通过 extended 标志区分）
    case RDP_SCANCODE_CODE(
        RDP_SCANCODE_LCONTROL): // 左/右 Ctrl（通过 extended 标志区分）
    case RDP_SCANCODE_CODE(
        RDP_SCANCODE_LSHIFT): // 左/右 Shift（通过 extended 标志区分）
    case RDP_SCANCODE_CODE(
        RDP_SCANCODE_LWIN): // 左/右 Win（通过 extended 标志区分）
      // 对于修饰键，使用 X11 的 KeySym 到 keycode 映射
      // 这样可以确保与当前系统的键盘布局一致
      keycode = (guint16)drd_x11_input_lookup_modifier_keycode(
          self, base_scancode, extended);
      break;
    default:
...
static void drd_x11_input_refresh_pointer_scale(DrdX11Input *self) {
  ...
  const guint32 stream_height =
      self->stream_height > 0 ? self->stream_height : 1u;
  const guint32 desktop_width =
      self->desktop_width > 0 ? self->desktop_width : 1u;
  const guint32 desktop_height =
      self->desktop_height > 0 ? self->desktop_height : 1u;
 ...
 * 逻辑：处理常见控制字符，ASCII 直接返回；合法 BMP/非 BMP 码点附加 0x01000000
 * 标志；非法返回 NoSymbol。 参数：codepoint Unicode 码点。 外部接口：X11 KeySym
 * 常量。
 */
static KeySym drd_x11_input_keysym_from_codepoint(gunichar codepoint) {
  switch (codepoint) {
